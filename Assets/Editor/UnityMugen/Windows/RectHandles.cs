...
namespace Anima2D
{
    public class RectHandles
    {
        ...
        public static Rect Do(ref Rect handleRect, ref Vector3 handlePosition, Color color, int controlID, bool usePivot = true, bool anchorPivot = false)
        {
            handleRect.center += new Vector2((handleRect.width / 2) * -1, (handleRect.height / 2) * -1);

            GUI.color = Color.white;
            Quaternion handleRectRotation = Quaternion.identity;

            RenderRect(handleRect, handlePosition, handleRectRotation, color, 0.05f, 0.8f);

            handleRect = MoveHandlesGUI(handleRect, ref handlePosition, handleRectRotation, controlID, anchorPivot || !usePivot);
        ...
        static Rect MoveHandlesGUI(Rect rect, ref Vector3 position, Quaternion rotation, int controlID, bool anchorPivot = false)
        {
